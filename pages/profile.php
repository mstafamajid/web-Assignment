...
$sqlstt = $conn->prepare("SELECT books.book_id, books.book_title as title, books.num_of_posts `num-of-posts`, books.image_path FROM books where books.user_id=?;
");
...
if ($result) {
    ...
    $sqlstt->bind_result($book_id,$title, $num_of_posts,$path);
    
    while ($sqlstt->fetch()) {
  
        array_push($bookdata, ['book_id'=>$book_id,'title' => $title, 'num-of-posts' => $num_of_posts, 'path'=>"../".$path]);
    }
}
...
<body>
 ...
 $sql2="SELECT count(*) as num FROM `likes` where user_id=$userid and is_liked='unlike';
 ";
 ...
 $row=$res2->fetch_assoc();
            ...
            <div class="numbook ">
                ...
                <h2>'.$userdata["num-of-book"].'</h2>
            </div>
            <div class="numpost">
                <h2 class=" num">No. posts</h2>
                <h2>'.$userdata["num-of-post"].'</h2>
            </div>
...
for ($i=0; $i < count($bookdata); $i++) { 
  ...
  <input type='hidden' name='book_info' value='".$bookdata[$i]['book_id']."'>
  <button type='submit' id='kak-mhm'>
